...
import com.trial.config.KafkaProperties;
...
class TrialAppKafkaResourceIT {
    ...
    void setup() {
        ...
        TrialAppKafkaResource kafkaResource = new TrialAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/trial-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/trial-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
